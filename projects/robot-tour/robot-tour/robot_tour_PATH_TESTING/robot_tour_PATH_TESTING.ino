...
int STRAIGHT_ENDING_SLOW = 45;
int STRAIGHT_ENDING_FAST = 65;
int TURN_SPEED = 80;
int TURN_SLOW = 50;

int FULL_TURN_TICKS = 740;
double TRANSLATIONAL_TOLERANCE = 2;
double FORWARD_LR_TOLERANCE = 0.01;
double TURN_TOLERANCE = 0.04;
double TURN_TIME = 1700;
...
void forward(double distance, double time) {
  targetX += distance * cos(robotTheta);
  targetY += distance * sin(robotTheta);
  ...
  //find theta offset
  while (getDistance(robotX, robotY, targetX, targetY) > TRANSLATIONAL_TOLERANCE) {
    printInfo();
    Serial.print("Distance :");
    double remainingDistance = getDistance(robotX, robotY, targetX, targetY);
    Serial.print(remainingDistance);
    Serial.println();
    updatePose();
    targetTheta = atan2(targetY - robotY, targetX - robotX);
    bool slowMode = getDistance(robotX, robotY, targetX, targetY) < 10;
...
void printInfo() {
  ...
  Serial.print("(rad) / ");
  Serial.print(targetTheta * (180 / PI));
  Serial.print(" (deg)");
  Serial.println();
}
...
void setup() {
  ...
  leds[0] = CRGB::Green;
  FastLED.show();  
  Serial.println("\nReady to start. Waiting for Trigger Press...");
  while (digitalRead(PIN_TRIGGER) == HIGH) {
    //wait
  }
  leds[0] = CRGB::Blue;
