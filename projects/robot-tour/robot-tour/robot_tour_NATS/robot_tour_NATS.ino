...
double TURN_TOLERANCE = 0.05;//if this is too low, it will miss turns.
...
void turnTo(double angle) {
  ...
  while (abs(angularDistance(robotTheta, targetTheta)) > TURN_TOLERANCE) {
    updatePose();
    printInfo();
    sign = angularDistance(robotTheta, targetTheta) > 0 ? 1 : -1;
    //Serial.println(angularDistance(robotTheta, targetAngle));
    double k = sign * TURN_SPEED;
    if (abs(angularDistance(robotTheta, targetTheta)) < 0.5) {
      k = sign * TURN_SLOW * min(1, abs(angularDistance(robotTheta, targetTheta) * 10));
    }
