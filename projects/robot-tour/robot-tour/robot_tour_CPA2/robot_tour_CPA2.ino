...
void setup() {
  ...
  //time: 62
  //end: 11
  forward(39 + 150, 7.5);//forward thru A to C
  turnTo(WEST, true);
  forward(50, 3);
  turnTo(SOUTH, true);
  forward(30, 2);
...
void findThetaOffset() {
  ...
  thetaOffset = atan(deltaY / deltaX);
}
...
void forward(double distance, double time) {
  targetX += distance * cos(targetTheta);
  targetY += distance * sin(targetTheta);
...
void printInfo() {
  ...
  Serial.print("(rad) / ");
  Serial.print((thetaOffset) * (180 / PI));
  Serial.print(" (deg)");
  Serial.println();
}
