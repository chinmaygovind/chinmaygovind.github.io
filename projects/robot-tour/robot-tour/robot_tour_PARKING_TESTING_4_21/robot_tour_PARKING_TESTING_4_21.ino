...
void readColorValues() {
  ...
  leftRed = map(leftRedFrequency, 40, 180, 255, 0);
  rightRedFrequency = pulseIn(RIGHT_OUT, LOW);
  rightRed = map(rightRedFrequency, 40, 220, 255, 0);
  //read GREEN
  ...
  leftGreen = map(leftGreenFrequency, 55, 225, 255, 0);
  rightGreenFrequency = pulseIn(RIGHT_OUT, LOW);
  rightGreen = map(rightGreenFrequency, 55, 225, 255, 0);
  //read BLUE
  digitalWrite(LEFT_S2, LOW);
  digitalWrite(LEFT_S3, HIGH);
  digitalWrite(RIGHT_S2, LOW);
  ...
  leftBlue = map(leftBlueFrequency, 65, 190, 255, 0);
  rightBlueFrequency = pulseIn(RIGHT_OUT, LOW);
  rightBlue = map(rightBlueFrequency, 65, 190, 255, 0);
...
void forward(double distance, double time) {
  ...
  double speed;
  if (time > 0) {
    speed = max(STRAIGHT_SLOW, min(255, SPEED_CONVERSION * (newDist / newTime)));//PWM / (cm/s) * cm/s
  } else {
    speed = STRAIGHT_SLOW;
  }
...
void park() {
  ...
  while (robotX < distance) {
    ...
    readColorValues();
    printColorValues();
    int slowSpeed = STRAIGHT_ENDING_SLOW;
    int fastSpeed = STRAIGHT_ENDING_FAST;
    if (angularDistance(robotTheta, targetTheta) < -FORWARD_LR_TOLERANCE) {
      //Serial.println("Correcting for left movement by steering right...");
      setMotors(fastSpeed, slowSpeed);
    } else if (angularDistance(robotTheta, targetTheta) > FORWARD_LR_TOLERANCE) {
    ...
    if (!passedTape) {
      if (!leftHitTape && colorValues[2] > 200 && colorValues[0] < 160) {
        leftHitTape = true;
        setLED(CRGB::Red);
      }
      if (!rightHitTape && colorValues[5] > 200 && colorValues[3] < 200) {
        rightHitTape = true;
        setLED(CRGB::Yellow);
      }
