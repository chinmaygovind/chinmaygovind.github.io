...
void setup() {
  ...
  robotTheta = 0;
  setSpeed(200);
  forward(39 + 150);//forward to A
  turnTo(WEST, true);//turn to A
  forward(130);//into A
  backward(130);//back to A
  turnTo(NORTH, false);//turn to B
  backward(150);//forward to B
  turnTo(WEST, true);//turn to B
  forward(50, 2.5);//forward to B
  turnTo(NORTH, false);//turn to B
  forward(30, 2);//into B
  backward(30, 2);//out of B
  turnTo(WEST, true);//turn to C
  forward(50, 2.5);//forward to C
  turnTo(NORTH, false);//turn north
  forward(50, 2.5);//forward to C
  turnTo(WEST, true);//turn left
  forward(50, 2.5);//forward to C
  turnTo(NORTH, false);
  forward(50, 2.5);//forward to C
  turnTo(EAST, false);//turn to C
  forward(30, 2);
  backward(30, 2);
  turnTo(NORTH, true);
  backward(50, 2.5);
  ...
  backward(42, 2.5);
  turnTo(WEST, true);
  forwardStall(41, 71, startMillis);
} 
